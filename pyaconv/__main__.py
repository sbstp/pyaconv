...
def ask_folders(journal, src_dir, dest_dir, encoder):
    ...
    for path in decisions:
        if decisions[path]:
            audio, other = fsutil.walk(Path(path), dest_dir, encoder.extension(), src_dir)
            audio_files.extend(audio)
            other_files.extend(other)

    # Compute the list of paths not yet visited.
    to_visit = (path for path in src_dir.iterdir() if path.is_dir() and str(path) not in decisions)
